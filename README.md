...
Generated from a Shamela `.bok` whose SHA-256 is recorded in `../data/v2/corpus/manifest.json`, so any run is verifiable against the same source.
...
`pipeline` needs the `.bok` file in `../data/source/isabah/`, or pass `--bok <path>` / set `BOK_PATH`. The whole deterministic pipeline runs in about 15 seconds.
...
Pipeline output lives in the sibling `data/` repo (`data/v2/corpus/` and `data/v2/reports/`). Regenerate with `npm run pipeline`. Read `samples/` to see the record shape without generating anything.
...
The Shamela `.bok` for al-Isabah, book id 9767, lives in `../data/source/isabah/`. Text is `windows-1256` and must be decoded before anything else touches it; lines are separated by bare carriage returns, which is a common source of silent parsing failures. The file's SHA-256 is written to `data/v2/corpus/manifest.json` on every ingest so results are attributable to an exact source.
